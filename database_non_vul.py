...
def load_vector_db_non_vul(db_path):
    load_dotenv()
...
def get_searching_results_non_vul(query, vector_db, embedding_model, res_num=2):
    """
    使用向量相似度搜索获取相似代码片段
    """
    try:
        # 1. 生成查询向量
        print(f"正在生成查询向量，查询长度: {len(query)}")
        query_embedding = embedding_model.embed_query(query)
        print("查询向量生成成功")

        # 2. 执行向量搜索
        print(f"开始执行相似度搜索，请求结果数: {res_num}")
        results = vector_db.similarity_search_by_vector(query_embedding, k=res_num)
        print(f"搜索完成，获得结果数: {len(results)}")

        # 3. 格式化结果
        searching_results = "\nResults:\n" + "\n\nResult:\n\n".join([doc.page_content for doc in results])
        print(f"结果格式化完成，总长度: {len(searching_results)}")
